...
namespace Social.Facebook
{
    public class FacebookClient
    {
        ...
        private const string PAGE_TOKEN = "EAAR8yzs1uVQBAKwg18FkCQkF45O2ZBtHl9FXSlAZCUHZAFDH8vGAYYKRXZBnBMZAFE2PSyBNsSZBcE4EZC0G7VAkZAAs2ZBZAZArJUIFhhtaS0weZAhD2bluPj5MYotIdzDDZCOIouZB1p8ffDKobc2osH9ZBf0aNwUyRyht3kZD";

        public async Task<Token> GetUserToken(string code, string redirectUri)
        {
            string tokenUrl =
                string.Format(
                    "{0}/oauth/access_token?client_id={1}&client_secret={2}&redirect_uri={3}&code={4}",
                    BASE_GRAPH_API_URL,
                    CLIENT_ID,
                    CLIENT_SECRET,
                    redirectUri,
        ...
        public async Task<Token> GetApplicationToken()
        {
            ...
            string url =
                string.Format(
                    "{0}/oauth/access_token?client_id={1}&client_secret={2}&grant_type=client_credentials",
                    BASE_GRAPH_API_URL,
                    CLIENT_ID,
                    CLIENT_SECRET
                    );

...
namespace Jax.Tests
{
    public abstract class JaxTestBase : AbpIntegratedTestBase<JaxTestModule>
    {
        ...
        protected void UsingDbContext(Action<JaxDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<JaxDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<JaxDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<JaxDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<JaxDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<JaxDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<JaxDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<JaxDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<JaxDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<JaxDbContext, Task<T>> func)
        {
            T result;

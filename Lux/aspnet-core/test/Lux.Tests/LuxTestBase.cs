...
namespace Lux.Tests
{
    public abstract class LuxTestBase : AbpIntegratedTestBase<LuxTestModule>
    {
        ...
        protected void UsingDbContext(Action<LuxDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<LuxDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<LuxDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<LuxDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<LuxDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<LuxDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<LuxDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<LuxDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<LuxDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<LuxDbContext, Task<T>> func)
        {
            T result;
